...
describe('Organization', () => {
    ...
    it('should add a SINGLE Organization on users/new POST', (done) => {
      chai.request(server)
        .post('/users/new')
        .send({
          name: 'Sylvain Laugier',
    ...
    it('should get a SINGLE Organization on users/:id GET', (done) => {
      chai.request(server)
        .get(`/users/${dummyId}`)
        .end((err, res) => {
          res.should.have.status(200);
          ...
          res.body.labels[0].should.equal('Organization');
          res.body.properties.should.have.property('name');
          res.body.properties.name.should.equal('Test Organization');
          res.body.properties.should.have.property('email');
          res.body.properties.email.should.equal('test_user@gmail.com');
          done();
        });
